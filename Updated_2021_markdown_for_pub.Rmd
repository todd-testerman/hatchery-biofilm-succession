...
```{r qpcr-total-load}
# Extract 16S copy numbers from sample metadata
sd_ws <- data.frame(sample_data(phy_wall_swabs_clean))
sd_ws$Copies_16S <- as.numeric(as.character(sd_ws$X16S_Copies))
sd_ws$Maturity_Days <- factor(sd_ws$Maturity_Days,
                              levels = c("9", "23", "38", "53", "65", "80"))
sd_ws$log_copies <- log10(sd_ws$Copies_16S)

# Kruskal-Wallis test across timepoints
kw_qpcr <- kruskal.test(log_copies ~ Maturity_Days, data = sd_ws)
kw_qpcr

# Pairwise Wilcoxon tests (Holm correction)
qpcr_wilcox <- sd_ws %>% wilcox_test(log_copies ~ Maturity_Days)
qpcr_wilcox
write.table(qpcr_wilcox, "qpcr_wilcox_ws.csv", sep = ",")

# Spearman correlation: biofilm age vs total bacterial load
cor_qpcr <- cor.test(as.numeric(as.character(sd_ws$Maturity_Days)),
                     sd_ws$log_copies, method = "spearman")
cor_qpcr

# Early vs Late comparison
sd_ws$Maturity_Group <- ifelse(as.numeric(as.character(sd_ws$Maturity_Days)) <= 38,
                               "Early", "Late")
wilcox.test(log_copies ~ Maturity_Group, data = sd_ws)

# Median copies per group
tapply(sd_ws$Copies_16S, sd_ws$Maturity_Group, median)

# Boxplot
qpcr_plot <- ggplot(sd_ws, aes(x = Maturity_Days, y = Copies_16S, color = Maturity_Days)) +
  geom_boxplot(show.legend = TRUE, outlier.size = 0.2, lwd = 0.25) +
  geom_point(size = 0.2) +
  scale_y_log10(labels = scales::comma) +
  theme_bw() +
  labs(color = "Age (Days)", y = "16S rRNA gene copies", x = "Biofilm Age (Days)") +
...
```{r fig6-important-taxa-plots, fig.width=10, fig.height=9}
# Ordered early-to-late by successional stage

# Early-stage dominants (peak at day 9, declining)
caulo_plot     <- make_taxon_boxplot(melted_genus, "Genus", "Caulobacter", c(0, 5), c(0, 1, 2, 3, 4))
flavo_plot     <- make_taxon_boxplot(melted_genus, "Genus", "Flavobacterium", c(0, 50), c(0, 1, 5, 10, 20, 30, 40))
sphingorh_plot <- make_taxon_boxplot(melted_genus, "Genus", "Sphingorhabdus", c(0, 10), c(0, 1, 2, 3, 4, 5, 10))

# Mid-succession specialists (peak day 23-38, declining later)
rhizor_plot    <- make_taxon_boxplot(melted_genus, "Genus", "Rhizorhapis", c(0, 20), c(0, 1, 5, 10, 20))
bdello_plot    <- make_taxon_boxplot(melted_genus, "Genus", "Bdellovibrio", c(0, 3), c(0, 0.5, 1, 1.5, 2))
methylo_plot   <- make_taxon_boxplot(melted_genus, "Genus", "Methylotenera", c(0, 20), c(0, 1, 5, 10, 15))

# Persistent colonizer (rises early, plateaus)
rhodo_plot     <- make_taxon_boxplot(melted_genus, "Genus", "Rhodobacteraceae Family", c(0, 25), c(0, 1, 5, 10, 20))

# Late-stage dominants (increasing throughout)
sapro_plot     <- make_taxon_boxplot(melted_genus, "Genus", "Saprospiraceae Family", c(0, 40), c(0, 1, 5, 10, 20, 30, 40))
halisco_plot   <- make_taxon_boxplot(melted_genus, "Genus", "Haliscomenobacter", c(0, 30), c(0, 1, 5, 10, 20, 30))
verruco_plot   <- make_taxon_boxplot(melted_genus, "Genus", "Verrucomicrobiaceae Family", c(0, 5), c(0, 1, 2, 3, 4, 5))
nanno_plot     <- make_taxon_boxplot(melted_genus, "Genus", "Nannocystis", c(0, 1.5), c(0, 0.25, 0.5, 0.75, 1))

# Composite Figure 6 (early -> mid -> persistent -> late)
Figure_6 <- (
  caulo_plot + flavo_plot + sphingorh_plot +
  rhizor_plot + bdello_plot + methylo_plot +
  rhodo_plot + sapro_plot + halisco_plot +
  verruco_plot + nanno_plot + plot_spacer()
) + plot_layout(ncol = 3, guides = "collect")
Figure_6

ggsave("Figures_all_maturity_days/Figure_6_important_taxa.png", width = 10, height = 9)
ggsave("Figures_all_maturity_days/Figure_6_important_taxa.tiff", width = 10, height = 9, dpi = 600)
```

```{r sfig6-sfig7-pathogen-boxplots}
# SFigure 6: F. columnare and SFigure 7: F. psychrophilum
...
ggsave("Figures_all_maturity_days/SFigure_6_Fc_boxplot.png", fc_plot, width = 6, height = 4)
ggsave("Figures_all_maturity_days/SFigure_6_Fc_boxplot.tiff", fc_plot, width = 6, height = 4, dpi = 300)
ggsave("Figures_all_maturity_days/SFigure_7_Fp_boxplot.png", fp_plot, width = 6, height = 4)
ggsave("Figures_all_maturity_days/SFigure_7_Fp_boxplot.tiff", fp_plot, width = 6, height = 4, dpi = 300)
```
...
ggsave("Figures_all_maturity_days/SFigure_3_PCA_biplot.png", width = 8, height = 6)
ggsave("Figures_all_maturity_days/SFigure_3_PCA_biplot.tiff", width = 8, height = 6, dpi = 300)
```
...
ggsave("Figures_all_maturity_days/SFigure_5_RF_25_asvs.png", width = 8, height = 6)
ggsave("Figures_all_maturity_days/SFigure_5_RF_25_asvs.tiff", width = 8, height = 6, dpi = 300)
```
